...
rename plan_choice_sim plan_choice_sim_all_active
rename chose_sim_dominated chose_sim_dominated_all_active

duplicates drop

save "$temp/all_active_sim_choice_dominated", replace

use "$temp/insurance_data_for_matlab", clear

merge m:1 indiv_id year using "$temp/baseline_sim_choice_dominated"

drop _merge

merge m:1 indiv_id year using "$temp/all_tool_sim_choice_dominated"

drop _merge

merge m:1 indiv_id year using "$temp/all_active_sim_choice_dominated"

save "$temp/sim_choice_dominated_compare", replace
