function [log_like] = ll4a(x,caseid,choice,price)
    ...
    betabar_test = x(1,1);
    betavar_test = x(1,2);
    %exp_betavar_test = exp(betavar_test);
   ...
   %integral_choice = quadv(Fnormexp,betabar_test - (5 * exp_betavar_test), betabar_test + (5 * exp_betavar_test));

   %find log likelihood
  % log_like_per_sit = log(integral_choice) - log(sqrt(2*pi*exp_betavar_test));
